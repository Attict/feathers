...
class TextureComponent extends Component {
    ...
    TextureComponent(this.texture, {this.source}) {
        if (source == null) {
            source = Rect.fromLTWH(0.0, 0.0, 50.0, 50.0); /// TODO: Make this the size of the image
        }
    }
    ...
    void render(Context context) {
        if (texture.image != null) {
            ...
            if (position == null || size == null) {
                print("FEATHER: The TextureComponent requires the PositionComponent and SizeComponent!");
                position = new PositionComponent(x: 0.0, y: 0.0);
                size = new SizeComponent(width: 50.0, height: 50.0); // TODO: Fixme to be the size of the image
            }

            var destination = new Rect.fromLTWH(position.x, position.y, size.width, size.height);

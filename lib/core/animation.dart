...
class Animation {
    ...
    void load() async {
        ...
        if (presets != null) {
            ...
            for (int i = presets['start']; i < presets['end']; i++) {
                ...
                addFrame(presets['duration'], Rect.fromLTWH(x * presets['width'], y * presets['height'], presets['width'], presets['height']));
            }

            if (presets['direction'] != null && presets['direction'] == AnimationOptions.TEETER) {
    ...
    void createFrames(double duration, int start, int end, double width, double height, {AnimationOptions direction}) {
